...
class SPNN(ln.nn.LossNN):
    ...
    def criterion(self, X, y):
        ...
        if ORIGINAL:
            Qslope = self.params['Qslope']
            Qincpt = self.params['Qincpt']
            Pincpt = self.params['Pincpt']
        else:
    ...
    def LBFGS_training(self, X, y, returnnp=False, lbfgs_step = 0):
        ...
        if ORIGINAL:
            ...
            optim_bd = LBFGS([param for param in self.qslope_net.parameters()] +
                            [param for param in self.qincpt_net.parameters()] +
                            [param for param in self.pincpt_net.parameters()], history_size=100,
                            max_iter=10,
                            tolerance_grad=1e-08, tolerance_change=1e-09,
                            line_search_fn="strong_wolfe")
        optim = optim_bd
